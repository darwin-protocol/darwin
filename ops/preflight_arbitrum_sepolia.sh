...
ARBITRUM_SEPOLIA_RPC_URL="${ARBITRUM_SEPOLIA_RPC_URL:-${DARWIN_RPC_URL:-https://sepolia-rollup.arbitrum.io/rpc}}"
SEPOLIA_RPC_URL="${SEPOLIA_RPC_URL:-https://ethereum-sepolia-rpc.publicnode.com}"
DARWIN_MIN_ARBITRUM_ETH_WEI="${DARWIN_MIN_ARBITRUM_ETH_WEI:-5000000000000000}" # 0.005 ETH
DARWIN_DEPLOYMENT_FILE="${DARWIN_DEPLOYMENT_FILE:-$ROOT/ops/deployments/arbitrum-sepolia.json}"
...
ARBITRUM_CHAIN_ID=""
if ARBITRUM_CHAIN_ID="$(cast chain-id --rpc-url "$ARBITRUM_SEPOLIA_RPC_URL" 2>/dev/null)"; then
  :
else
  ARBITRUM_CHAIN_ID=""
fi
...
ARBITRUM_BALANCE_WEI="0"
SEPOLIA_BALANCE_WEI="0"
if [[ -n "$DEPLOYER_ADDRESS" && -n "$ARBITRUM_CHAIN_ID" ]]; then
  ARBITRUM_BALANCE_WEI="$(cast balance "$DEPLOYER_ADDRESS" --rpc-url "$ARBITRUM_SEPOLIA_RPC_URL")"
fi
...
READY=1
if [[ "$ARBITRUM_CHAIN_ID" != "421614" ]]; then
  READY=0
fi
...
if [[ "$ARBITRUM_BALANCE_WEI" -lt "$DARWIN_MIN_ARBITRUM_ETH_WEI" ]]; then
  READY=0
fi

echo "DARWIN Arbitrum Sepolia preflight"
echo "  arbitrum_rpc_url:     $ARBITRUM_SEPOLIA_RPC_URL"
echo "  arbitrum_chain_id:    ${ARBITRUM_CHAIN_ID:-unreachable}"
...
echo "  arbitrum_balance_eth: $(to_eth "$ARBITRUM_BALANCE_WEI")"
echo "  sepolia_balance_eth:  $(to_eth "$SEPOLIA_BALANCE_WEI")"
echo "  min_arbitrum_eth:     $(to_eth "$DARWIN_MIN_ARBITRUM_ETH_WEI")"
echo "  deployment_artifact:  $DARWIN_DEPLOYMENT_FILE"
echo "  bond_asset_mode:      $(if [[ "${DARWIN_DEPLOY_BOND_ASSET_MOCK:-0}" == "1" ]]; then echo mock; else echo external; fi)"
...
if [[ "$ARBITRUM_BALANCE_WEI" -lt "$DARWIN_MIN_ARBITRUM_ETH_WEI" ]]; then
  echo "  balance_status:       insufficient Arbitrum Sepolia ETH"
  if [[ "$SEPOLIA_BALANCE_WEI" -gt 0 ]]; then
    echo "  funding_hint:         bridge Sepolia ETH into Arbitrum Sepolia for the deployer"
  fi
else
