...
require_env DARWIN_REFERENCE_MARKET_BASE_TOKEN
require_env DARWIN_REFERENCE_MARKET_QUOTE_TOKEN
...
export DARWIN_REFERENCE_MARKET_FILE="${DARWIN_REFERENCE_MARKET_FILE:-$ROOT/ops/deployments/.${deployment_basename}.market.json}"

mkdir -p "$(dirname "$DARWIN_REFERENCE_MARKET_FILE")"
...
cd "$ROOT/contracts"
forge script script/DeployReferenceMarket.s.sol:DeployReferenceMarket \
...
python3 - "$DARWIN_DEPLOYMENT_FILE" "$DARWIN_REFERENCE_MARKET_FILE" <<'PY'
import json
import sys
...
market_deployment = json.loads(market_path.read_text())

contracts = deployment.setdefault("contracts", {})
contracts.update(market_deployment.get("contracts", {}))
deployment["market"] = market_deployment
...
echo "DARWIN reference market merged into deployment artifact."
echo "  deployment: $DARWIN_DEPLOYMENT_FILE"
echo "  market_json: $DARWIN_REFERENCE_MARKET_FILE"
